...
class UserController(private val userService: UserService) {
    ...
    @PatchMapping("/{id}")
    fun patch(@PathVariable id: Long, @RequestBody dto: UserPatchDTO): ResponseEntity<UserResponseDTO> {
        val updatedUser = userService.patch(id, dto)
        return ResponseEntity.status(HttpStatus.OK).body(updatedUser)
    }

    @DeleteMapping("/{id}")
    fun delete(@PathVariable id: Long): ResponseEntity<UserResponseDTO> {
